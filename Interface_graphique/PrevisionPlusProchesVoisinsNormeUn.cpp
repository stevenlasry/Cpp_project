...
// 1. Définition d'une fonction qui retourne la norme un entre deux vecteurs
...
QVector< QVector<double> > PrevisionPlusProchesVoisinsNormeUn::Lplusprochesvoisins(QVector<double> const& donnees, int k, int l, int t, int h)
{
   ...
   // c'est à dire dans chaque colonne 1. La distance euclidienne 2. L'observation 3. Les h observations suivantes
    ...
    for (int g(0) ; g < l ; ++g)
       ...
       {x[g]=donnees[i+g-1];
        ...
        // La première ligne de la matrice contient la distance en norme un
...
QVector<double> PrevisionPlusProchesVoisinsNormeUn::KLmax(QVector<double> const& donnees, int t, int h)
{
    ...
    double kmax(0);
    kmax=qMin(11,t-h+1);
    // Nous fixons un entier K
for(int k(2) ; k <kmax; ++k)
{
    // Nous créons un vecteur qui contiendra la valeur de L qui maximise l'estimation KL à K fixé ainsi que son score
    QVector<double> x(3);
    x[0]=1000000;
    x[1]=k;
    x[2]=1;
    ...
    {
    double scoreKL(0);
    // Nous avons fixé K et L. Nous cherchons maintenant à calculer le score de cette technique d'estimation
